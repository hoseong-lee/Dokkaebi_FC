...
<template>
  <section v-if="totalEndorsements > 0" class="bg-white dark:bg-zinc-800 rounded-2xl shadow p-6 space-y-4">
    <div class="flex items-start justify-between gap-3">
      <div>
        <h2 class="font-bold text-navy dark:text-zinc-100 flex items-center gap-2">⭐ 스킬 평판</h2>
    ...
    <div v-if="top3.length">
      <p class="text-[10px] text-gray-400 dark:text-zinc-500 font-bold uppercase tracking-wide mb-2">TOP 3 강점</p>
      <div class="space-y-1.5">
        <div v-for="(s, i) in top3" :key="s.id" class="flex items-center gap-3">
          <span class="text-[10px] font-bold text-gray-400 dark:text-zinc-500 w-5">{{ ['🥇','🥈','🥉'][i] }}</span>
          ...
          <span class="text-xs font-bold text-onyx dark:text-zinc-100 tabular-nums w-12 text-right">{{ s.count }}회</span>
        </div>
      </div>
    </div>
    ...
    <details v-if="sortedAll.length > 3" class="text-xs">
      <summary class="cursor-pointer text-gray-500 dark:text-zinc-400 font-semibold py-1">전체 스킬 분포 ({{ sortedAll.length }}종)</summary>
    ...
    <p class="text-[10px] text-gray-400 dark:text-zinc-500 leading-relaxed">
