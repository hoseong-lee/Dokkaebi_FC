...
<template>
  <ol v-if="sorted.length" class="space-y-3">
    <li v-for="(ev, i) in sorted" :key="i" class="flex items-start gap-3">
      <span class="text-xs font-mono text-gray-400 w-8 text-right pt-0.5">{{ ev.minute }}'</span>
      ...
      <div class="text-sm">
        ...
        <p v-if="ev.type === 'goal' && ev.assistPlayerId" class="text-xs text-gray-500">
          도움 {{ name(ev.assistPlayerId) }}
        </p>
      </div>
    </li>
  </ol>
  <p v-else class="text-sm text-gray-400 text-center py-4">기록된 이벤트가 없습니다.</p>
</template>
