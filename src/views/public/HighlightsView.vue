...
<template>
  <div>
    <div class="mb-4">
      <h1 class="text-xl font-bold text-navy">🏅 베스트 골 모음</h1>
      <p class="text-xs text-gray-500 mt-1">경기 영상 중 "베스트 골" 라벨 단 클립들만 모아보기</p>
    </div>

    <!-- 시즌 필터 -->
    <select v-model="scope" class="w-full border rounded-lg px-3 py-2 text-sm mb-4 bg-white">
      ...
      >
        ...
        <div class="flex items-center gap-3 p-4 border-b border-gray-100">
          ...
          <div class="flex-1 min-w-0">
            <p class="font-bold text-navy truncate">{{ h.label }}</p>
        ...
        <div class="flex gap-2 p-3 bg-gray-50">
          <RouterLink :to="`/matches/${h.matchId}`" class="flex-1 text-center text-xs py-2 rounded-lg bg-navy/10 text-navy font-semibold hover:bg-navy/20">
            🏟 경기 상세
          </RouterLink>
          ...
          <button
            type="button"
            class="flex-1 text-xs py-2 rounded-lg bg-yellow-300 text-onyx font-bold hover:bg-yellow-400"
            @click="copyShare(h)"
          >💬 단톡 공유
          </button>
        </div>
