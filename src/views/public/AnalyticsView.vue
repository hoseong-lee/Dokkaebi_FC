...
<template>
  <div class="pb-8">
    <div class="mb-4">
      <h1 class="text-xl font-bold text-navy flex items-center gap-2">📊 <span>승률 분석</span></h1>
      <p class="text-xs text-gray-500 mt-1">포메이션·선수·상대팀·라인업 조합별 승률 인사이트</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow p-4 mb-4">
      ...
      <div class="grid grid-cols-4 gap-2 text-center">
        <div><p class="text-[10px] text-gray-500">경기</p><p class="text-xl font-bold text-navy">{{ overall.played }}</p></div>
        <div><p class="text-[10px] text-blue-500">승</p><p class="text-xl font-bold text-blue-600">{{ overall.wins }}</p></div>
        <div><p class="text-[10px] text-gray-500">무</p><p class="text-xl font-bold text-gray-600">{{ overall.draws }}</p></div>
        <div><p class="text-[10px] text-rose-500">패</p><p class="text-xl font-bold text-rose-600">{{ overall.losses }}</p></div>
      </div>
      <div class="mt-2 text-center">
        <span class="text-xs text-gray-500">평균 승률 </span>
        <span class="text-lg font-bold text-navy">{{ pct(overall.winRate) }}</span>
        <span class="text-xs text-gray-400 mx-2">·</span>
        <span class="text-xs text-gray-500">평균 득점 {{ overall.avgGf.toFixed(1) }} / 실점 {{ overall.avgGa.toFixed(1) }}</span>
      </div>
    </div>

    <!-- 4탭 -->
    <div class="grid grid-cols-4 gap-1 bg-white rounded-xl p-1 mb-4 shadow-sm text-xs">
      <button type="button" class="py-2 rounded-lg font-medium transition-colors" :class="tab === 'formation' ? 'bg-navy text-white shadow' : 'text-gray-500'" @click="tab = 'formation'">📐 포메이션</button>
      <button type="button" class="py-2 rounded-lg font-medium transition-colors" :class="tab === 'player' ? 'bg-navy text-white shadow' : 'text-gray-500'" @click="tab = 'player'">⭐ 선수</button>
      <button type="button" class="py-2 rounded-lg font-medium transition-colors" :class="tab === 'opponent' ? 'bg-navy text-white shadow' : 'text-gray-500'" @click="tab = 'opponent'">🆚 상대팀</button>
      <button type="button" class="py-2 rounded-lg font-medium transition-colors" :class="tab === 'duo' ? 'bg-navy text-white shadow' : 'text-gray-500'" @click="tab = 'duo'">💑 듀오</button>
    </div>

    <LoadingSpinner v-if="loading" />

    <div v-else-if="overall.played === 0" class="bg-white rounded-2xl shadow p-8 text-center text-gray-400">
    ...
    <div v-else-if="tab === 'formation'" class="space-y-3">
      <div v-if="formations.length === 0" class="text-center text-sm text-gray-400 py-8">
        포메이션이 입력된 경기가 없어요.
      </div>
      <article v-for="f in formations" :key="f.formation" class="bg-white rounded-2xl shadow p-4">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-2">
            <span class="text-xl font-bold text-navy tabular-nums">{{ f.formation }}</span>
            <span class="text-xs text-gray-400">{{ f.played }}경기</span>
          </div>
          <div class="text-right">
            <p class="text-2xl font-bold text-navy">{{ pct(f.winRate) }}</p>
            <p class="text-[10px] text-gray-400">승률</p>
          </div>
        </div>
        <!-- 승무패 분포 막대 -->
        ...
        <div class="flex items-center justify-between text-xs">
          ...
          <span class="text-gray-500">{{ f.draws }}무</span>
          <span class="text-rose-500 font-semibold">{{ f.losses }}패</span>
          <span class="text-gray-400">·</span>
          <span class="text-gray-600">평균 {{ f.avgGf.toFixed(1) }}득 / {{ f.avgGa.toFixed(1) }}실 (GD <span :class="f.gd >= 0 ? 'text-emerald-600' : 'text-rose-600'">{{ f.gd > 0 ? '+' : '' }}{{ f.gd }}</span>)</span>
        </div>
      </article>
    </div>
    ...
    <div v-else-if="tab === 'player'" class="space-y-4">
      ...
      <div v-if="impacts.length === 0" class="text-center text-sm text-gray-400 py-8">
        분석 가능한 선수가 없어요.
      </div>

      <section v-else class="bg-white rounded-2xl shadow p-4">
        <p class="text-sm font-bold text-navy mb-3">🏆 Win-Rate Impact TOP</p>
        <ul class="space-y-2.5">
          <li v-for="(it, idx) in topImpact" :key="it.playerId" class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              <RouterLink :to="`/players/${it.playerId}`" class="text-sm font-semibold text-onyx hover:underline truncate">{{ it.player.name }}</RouterLink>
      ...
      <section v-if="bottomImpact.length" class="bg-white rounded-2xl shadow p-4">
        ...
        <ul class="space-y-2.5">
          <li v-for="it in bottomImpact" :key="`bot-${it.playerId}`" class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              <RouterLink :to="`/players/${it.playerId}`" class="text-sm font-medium text-onyx hover:underline truncate">{{ it.player.name }}</RouterLink>
              <p class="text-[10px] text-gray-400">출전 {{ it.withSelf.played }} · 결장 {{ it.withoutSelf.played }}</p>
            </div>
            <span class="text-xs font-bold px-2 py-1 rounded-full tabular-nums" :class="impactTone(it.impact)">{{ signed(it.impact) }}</span>
          </li>
        </ul>
    ...
    <div v-else-if="tab === 'opponent'" class="space-y-3">
      <div v-if="opponents.length === 0" class="text-center text-sm text-gray-400 py-8">상대팀 데이터가 없어요.</div>
      <article v-for="o in opponents" :key="o.opponent" class="bg-white rounded-2xl shadow p-4">
        <div class="flex items-center justify-between mb-2">
          <div class="flex-1 min-w-0">
            <p class="font-bold text-navy truncate">vs {{ o.opponent }}</p>
            <p class="text-[11px] text-gray-400">{{ o.played }}전 {{ o.wins }}승 {{ o.draws }}무 {{ o.losses }}패</p>
          </div>
          <div class="text-right">
            <p class="text-xl font-bold text-navy">{{ pct(o.winRate) }}</p>
            <p class="text-[10px] text-gray-400">승률</p>
          </div>
        </div>
        ...
        <p class="text-xs text-gray-600">
          득실 <span class="font-semibold text-onyx">{{ o.gf }}</span> : <span class="font-semibold text-onyx">{{ o.ga }}</span>
    ...
    <div v-else class="space-y-3">
      ...
      <div v-if="duos.length === 0" class="text-center text-sm text-gray-400 py-8">
        분석 가능한 듀오가 없어요.
      </div>
      <article v-for="(d, i) in duos" :key="`${d.pair[0].id}-${d.pair[1].id}`" class="bg-white rounded-2xl shadow p-3 flex items-center gap-2">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-[10px] text-gray-400">{{ d.played }}경기 · {{ d.wins }}승 {{ d.draws }}무 {{ d.losses }}패</p>
        </div>
        <div class="text-right shrink-0">
          <p class="text-base font-bold text-navy">{{ pct(d.winRate) }}</p>
          <p class="text-[10px] text-gray-400">승률</p>
        </div>
      </article>
    </div>
